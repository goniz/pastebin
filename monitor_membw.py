...
class ProcessMemStats:
    ...
    numa_anon_pages: Dict[int, int] = field(default_factory=dict)  # Anonymous pages per node
    numa_file_pages: Dict[int, int] = field(default_factory=dict)  # File-backed pages per node
    numa_heap_pages: Dict[int, int] = field(default_factory=dict)  # Heap pages per node
    numa_stack_pages: Dict[int, int] = field(default_factory=dict)  # Stack pages per node
...
class MemoryBandwidthMonitor:
    ...
    def read_zoneinfo(self) -> Dict[str, ZoneInfo]:
        ...
        zone_pattern = r'Node\s+(\d+),\s+zone\s+(\w+)(.*?)(?=Node\s+\d+,\s+zone|\Z)'
    ...
    def calculate_zone_lock_metrics(self, rates: Dict, pcp_stats: Dict[int, PCPStats]) -> Dict:
        ...
        # Get PCP stats for Node 0 (where tracking_3d runs)
        node0_pcp = pcp_stats.get(0)
        if node0_pcp and node0_pcp.batch > 0:
            # Estimate zone lock acquisitions per second
            # Each batch refill requires acquiring zone->lock
            estimated_lock_acq = alloc_rate / node0_pcp.batch
            metrics['zone_lock_acq_per_sec'] = estimated_lock_acq
            
            # PCP utilization - how full are the PCPs on average?
            if node0_pcp.high > 0 and node0_pcp.num_cpus > 0:
                avg_pcp_count = node0_pcp.total_count / node0_pcp.num_cpus
                metrics['pcp_utilization_pct'] = (avg_pcp_count / node0_pcp.high) * 100
            else:
                metrics['pcp_utilization_pct'] = 0
            
            # Time to exhaust PCP per CPU (assuming uniform distribution)
            cpus_on_node = node0_pcp.num_cpus if node0_pcp.num_cpus > 0 else 64
            per_cpu_alloc_rate = alloc_rate / cpus_on_node
            if per_cpu_alloc_rate > 0 and node0_pcp.high > 0:
                metrics['pcp_exhaust_time_ms'] = (node0_pcp.high / per_cpu_alloc_rate) * 1000
            else:
            ...
            metrics['lock_contention_pressure'] = min(100, lock_busy_fraction * 100 * cpus_on_node)
            
            metrics['pcp_high'] = node0_pcp.high
            metrics['pcp_batch'] = node0_pcp.batch
            metrics['pcp_total_cached'] = node0_pcp.total_count
        else:
    ...
    def _read_process_stats(self, pid: int) -> Optional[ProcessMemStats]:
        """Read stats for a single process"""
                    ...
                    for part in parts:
                        if part.startswith('N') and '=' in part:
                            try:
                                ...
                                stats.numa_pages[node_id] = stats.numa_pages.get(node_id, 0) + page_count
                                stats.numa_total_pages += page_count
                                
                                # Categorize by mapping type
                                if is_anon or is_heap:
                                    stats.numa_anon_pages[node_id] = stats.numa_anon_pages.get(node_id, 0) + page_count
                                if is_heap:
                                    stats.numa_heap_pages[node_id] = stats.numa_heap_pages.get(node_id, 0) + page_count
                                if is_stack:
                                    stats.numa_stack_pages[node_id] = stats.numa_stack_pages.get(node_id, 0) + page_count
                                if not is_anon and not is_heap and not is_stack:
                                    stats.numa_file_pages[node_id] = stats.numa_file_pages.get(node_id, 0) + page_count
                            except (ValueError, IndexError):
                                pass
        except (IOError, PermissionError):
            pass
    ...
    def calculate_bandwidth_rates(self, current: VmStats, prev: VmStats, 
        ...
        swap_activity = (
            (current.pswpin - prev.pswpin) + 
            (current.pswpout - prev.pswpout)
        ) / elapsed
        rates['swap_activity_s'] = swap_activity
        rates['memory_pressure'] = min(100, rates['major_faults_s'] + swap_activity * 10)
        ...
        rates['compact_stall_s'] = (current.compact_stall - prev.compact_stall) / elapsed
        
        # Direct reclaim rates (indicates memory pressure forcing synchronous reclaim)
        rates['pgsteal_direct_s'] = (current.pgsteal_direct - prev.pgsteal_direct) / elapsed
        rates['pgscan_direct_s'] = (current.pgscan_direct - prev.pgscan_direct) / elapsed
        
        return rates
    
    def calculate_node_bandwidth(self, current_stats: Dict[int, Dict[str, int]],
                                  prev_stats: Dict[int, Dict[str, int]],
                                  elapsed: float) -> Dict[int, Dict[str, float]]:
        ...
        for node_id in self.numa_nodes:
            ...
            rates = {}
            for key in ['numa_hit', 'numa_miss', 'numa_foreign', 'local_node', 'other_node']:
    ...
    def merge_processes_by_name(self, processes: List[ProcessMemStats]) -> List[ProcessMemStats]:
        ...
        for proc in processes:
            if proc.name in merged:
                ...
                for node_id, pages in proc.numa_pages.items():
                    existing.numa_pages[node_id] = existing.numa_pages.get(node_id, 0) + pages
                existing.numa_total_pages += proc.numa_total_pages
                for node_id, pages in proc.numa_anon_pages.items():
                    existing.numa_anon_pages[node_id] = existing.numa_anon_pages.get(node_id, 0) + pages
                for node_id, pages in proc.numa_file_pages.items():
                    existing.numa_file_pages[node_id] = existing.numa_file_pages.get(node_id, 0) + pages
                for node_id, pages in proc.numa_heap_pages.items():
                    existing.numa_heap_pages[node_id] = existing.numa_heap_pages.get(node_id, 0) + pages
                for node_id, pages in proc.numa_stack_pages.items():
                    existing.numa_stack_pages[node_id] = existing.numa_stack_pages.get(node_id, 0) + pages
                existing.count += 1
            else:
    ...
    def get_top_memory_processes(self, processes: List[ProcessMemStats],
                                  prev_stats: Dict,
                                  elapsed: float) -> List[Tuple[ProcessMemStats, Dict]]:
        ...
        for proc in processes:
            ...
            if prev and elapsed > 0:
                ...
                rates['minor_faults_s'] = max(0, proc.minor_faults - prev.minor_faults) / elapsed
                rates['major_faults_s'] = max(0, proc.major_faults - prev.major_faults) / elapsed
                ...
                rates['alloc_mb_s'] = (rates['alloc_pages_s'] * PAGE_SIZE) / (1024 * 1024)
                
                # I/O rates (physical disk)
                rates['io_read_mb_s'] = max(0, proc.read_bytes - prev.read_bytes) / (1024 * 1024 * elapsed)
                rates['io_write_mb_s'] = max(0, proc.write_bytes - prev.write_bytes) / (1024 * 1024 * elapsed)
                
                # Virtual I/O rates (includes page cache)
                rates['vio_read_mb_s'] = max(0, proc.rchar - prev.rchar) / (1024 * 1024 * elapsed)
                rates['vio_write_mb_s'] = max(0, proc.wchar - prev.wchar) / (1024 * 1024 * elapsed)
                ...
                rates['cpu_user_pct'] = max(0, (proc.utime - prev.utime) / hz) / elapsed * 100
                rates['cpu_sys_pct'] = max(0, (proc.stime - prev.stime) / hz) / elapsed * 100
            ...
            if proc.numa_total_pages > 0:
                ...
                primary_node = max(proc.numa_pages.keys(), key=lambda n: proc.numa_pages.get(n, 0)) if proc.numa_pages else 0
                ...
                rates['numa_mb'] = {n: (p * PAGE_SIZE) / (1024 * 1024) for n, p in proc.numa_pages.items()}
            else:
        ...
        return process_rates[:self.top_n]
    ...
    def run_curses(self, stdscr):
        ...
        if self.merge_by_name:
            prev_proc_stats = {p.name: p for p in self.merge_processes_by_name(initial_processes)}
        else:
        ...
        while True:
            try:
                ...
                # Display
                self._draw_screen(
                    stdscr, rates, node_rates, node_meminfo, 
                    meminfo, top_procs, iteration,
                ...
                if self.merge_by_name:
                    prev_proc_stats = {p.name: p for p in self.merge_processes_by_name(current_processes)}
                else:
    ...
    def _draw_screen(self, stdscr, rates: Dict, node_rates: Dict, 
                     node_meminfo: Dict, meminfo: Dict,
                     top_procs: List, iteration: int,
        ...
        row += 1
        stdscr.addstr(row, 0, f" Refresh: {self.interval}s | Press 'q' to quit | Iteration: {iteration}")
        row += 1
        stdscr.addstr(row, 0, "=" * min(width-1, 80))
        row += 2
        ...
        if meminfo:
            ...
            row += 1
            stdscr.addstr(row, 0, f"  Total: {self.format_size(mem_total)}  "
        ...
        if rates:
            ...
            stdscr.addstr(row, 0, "MEMORY BANDWIDTH ESTIMATION", curses.A_BOLD | curses.color_pair(5))
            row += 1
            ...
            stdscr.addstr(row, 0, f"  Page I/O: Read {rates.get('read_mb_s', 0):.1f} MB/s | "
                                  f"Write {rates.get('write_mb_s', 0):.1f} MB/s | "
                                  f"Total {total_bw:.1f} MB/s {bw_trend}", color)
            row += 2
        
        # Per-NUMA Node Statistics
        stdscr.addstr(row, 0, "PER-NUMA NODE STATISTICS", curses.A_BOLD | curses.color_pair(5))
        row += 1
        ...
        for node_id in self.numa_nodes:
            ...
            stdscr.addstr(f"Local {local_mb:.1f} MB/s | Remote {remote_mb:.1f} MB/s | "
                         f"Total {total_mb:.1f} MB/s {trend}", color)
            row += 1
            stdscr.addstr(row, 0, f"          Mem: {self.format_size(nm.mem_total - nm.mem_free)} used / "
        ...
        if rates:
            ...
            row += 1
            stdscr.addstr(row, 0, f"  Local: {self.format_rate(rates.get('numa_local_s', 0))} {local_trend} | "
                                  f"Remote: {self.format_rate(rates.get('numa_other_s', 0))} {remote_trend} | "
                                  f"Locality: ", curses.A_NORMAL)
        ...
        if rates:
            ...
            stdscr.addstr(row, 0, "PAGE FAULTS & MEMORY PRESSURE", curses.A_BOLD | curses.color_pair(5))
            row += 1
            stdscr.addstr(row, 0, f"  Minor Faults: {self.format_rate(rates.get('minor_faults_s', 0))} {fault_trend} | "
                                  f"Major Faults: {self.format_rate(rates.get('major_faults_s', 0))} {major_trend}")
            row += 1
            stdscr.addstr(row, 0, f"  Page Alloc: {self.format_rate(rates.get('alloc_pages_s', 0))} | "
        ...
        if zone_lock_metrics and row < height - 15:
            ...
            stdscr.addstr(row, 0, "ZONE LOCK & PCP METRICS", curses.A_BOLD | curses.color_pair(5))
            row += 1
            ...
            stdscr.addstr(row, 0, f"  PCP Config: high={pcp_high} batch={pcp_batch} | "
            ...
            stdscr.addstr(row, 0, f"  Zone Lock Acq: {self.format_rate(lock_acq)} | "
            ...
            if lock_pressure > 30:
                stdscr.addstr(" [HIGH - consider NUMA separation]", curses.color_pair(3))
            row += 1
            ...
            if allocstall > 0 or compact_stall > 0:
                stdscr.addstr(row, 0, f"  Alloc Stalls: {self.format_rate(allocstall)} | "
        ...
        # Top Processes
        if self.merge_by_name:
            stdscr.addstr(row, 0, "TOP PROCESSES BY MEMORY ACTIVITY (merged by name)", curses.A_BOLD | curses.color_pair(5))
        else:
            stdscr.addstr(row, 0, "TOP PROCESSES BY MEMORY ACTIVITY", curses.A_BOLD | curses.color_pair(5))
        row += 1
        ...
        for proc, proc_rates in top_procs:
            ...
            if self.merge_by_name:
                ...
                proc_line = (f"  {proc.count:>4} {proc.name[:12]:<12} "
                ...
                proc_line = (f"  {proc.pid:>7} {proc.name[:12]:<12} "
                            f"{self.format_size(proc.rss):>7} "
                            f"{proc_rates.get('minor_faults_s', 0):>7.0f} "
                            f"{alloc_str:>7} "
                            f"{syscall_str:>7} "
        ...
        # Footer
        stdscr.addstr(height-1, 0, " Legend: ↑ increasing | ↓ decreasing | → stable | ? insufficient data"[:width-1])
    ...
    def run_simple(self):
        ...
        try:
            while True:
                ...
                mem_total = meminfo.get('MemTotal', 0)
                mem_available = meminfo.get('MemAvailable', 0)
                print(f"SYSTEM MEMORY: {self.format_size(mem_available)} available / "
                      f"{self.format_size(mem_total)} total\n")
                
                # Bandwidth
                if rates:
                    bw_trend = self.trends['mem_bandwidth'].trend()
                    print(f"BANDWIDTH: Read {rates.get('read_mb_s', 0):.1f} MB/s | "
                          f"Write {rates.get('write_mb_s', 0):.1f} MB/s | "
                ...
                for node_id in self.numa_nodes:
                    ...
                    print(f"  Node {node_id}: Local {nr.get('local_mb_s', 0):.1f} MB/s | "
                ...
                # NUMA locality
                if rates:
                    print(f"NUMA LOCALITY: {rates.get('numa_local_pct', 100):.1f}% local accesses\n")
                
                # Page faults
                if rates:
                    print(f"PAGE FAULTS: Minor {self.format_rate(rates.get('minor_faults_s', 0))} | "
                ...
                if zone_lock_metrics:
                    ...
                    pcp_batch = zone_lock_metrics.get('pcp_batch', 0)
                    pcp_cached = zone_lock_metrics.get('pcp_total_cached', 0)
                    print(f"  PCP Config: high={pcp_high} batch={pcp_batch} | Cached: {pcp_cached:,} pages")
                    
                    lock_acq = zone_lock_metrics.get('zone_lock_acq_per_sec', 0)
                    pcp_exhaust = zone_lock_metrics.get('pcp_exhaust_time_ms', 0)
                    exhaust_str = f"{pcp_exhaust:.1f}ms" if pcp_exhaust != float('inf') else "N/A"
                    print(f"  Zone Lock Acq: {self.format_rate(lock_acq)} | PCP Exhaust Time: {exhaust_str}")
                    ...
                    print(f"  Lock Contention Pressure: {lock_pressure:.1f}/100 {lock_trend}{pressure_indicator}")
                    
                    allocstall = rates.get('allocstall_s', 0)
                    if allocstall > 0:
                        print(f"  Alloc Stalls: {self.format_rate(allocstall)} [CONTENTION DETECTED]")
                    print()
                
                # Top processes
                if self.merge_by_name:
                    print("TOP PROCESSES (merged by name):")
                    print(f"  {'#':>4} {'Name':<12} {'RSS':>7} {'MinF/s':>7} {'Alloc':>7} {'Sys/s':>7} {'CPU%':>5} {'NUMA Distribution':<20}")
                    print(f"  {'-'*4} {'-'*12} {'-'*7} {'-'*7} {'-'*7} {'-'*7} {'-'*5} {'-'*20}")
                else:
                    print("TOP PROCESSES:")
                    print(f"  {'PID':>7} {'Name':<12} {'RSS':>7} {'MinF/s':>7} {'Alloc':>7} {'Sys/s':>7} {'CPU%':>5} {'NUMA Distribution':<20}")
                    print(f"  {'-'*7} {'-'*12} {'-'*7} {'-'*7} {'-'*7} {'-'*7} {'-'*5} {'-'*20}")
                ...
                for proc, proc_rates in top_procs[:10]:
                    ...
                    if self.merge_by_name:
                        print(f"  {proc.count:>4} {proc.name[:12]:<12} "
                              f"{self.format_size(proc.rss):>7} "
                              f"{proc_rates.get('minor_faults_s', 0):>7.0f} "
                              f"{alloc_str:>7} "
                              f"{syscall_str:>7} "
                        ...
                        print(f"  {proc.pid:>7} {proc.name[:12]:<12} "
                              f"{self.format_size(proc.rss):>7} "
                              f"{proc_rates.get('minor_faults_s', 0):>7.0f} "
                              f"{alloc_str:>7} "
                              f"{syscall_str:>7} "
